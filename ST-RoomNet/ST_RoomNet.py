...
class ProcessorConfig:
    ...
    # 입력 경로 설정
    val_path: str = '/content/drive/MyDrive/Colab Notebooks/Model/ShowRoom/Input/Images'  # chrome extension에서 저장한 이미지
    pose_path: str = '/content/drive/MyDrive/Colab Notebooks/Model/ShowRoom/Input/Pose/pose.txt'
    ref_img_path: str = 'ref_img2.png'
    weight_path: str = '/content/drive/MyDrive/Colab Notebooks/Model/ShowRoom/ST-RoomNet/weights/Weight_ST_RroomNet_ConvNext.h5'
    
    # 출력 경로 설정
    save_path: str = '/content/drive/MyDrive/Colab Notebooks/Model/ShowRoom/Input/ST'
...
class ShowRoomProcessor:
    ...
    def __init__(self, config: Optional[ProcessorConfig] = None):
        ...
        Args:
            config: 프로세서 설정 객체. None인 경우 기본 설정 사용
        """
    ...
    def _load_data(self) -> None:
        ...
        self.img_names = [os.path.splitext(f)[0] for f in img_filenames]  # ['0', '1', '2', ...]
        ...
        # 이미지 이름과 포즈 수가 같아야 함
        assert len(self.img_names) == len(pose_list), "이미지 수와 포즈 수가 일치하지 않습니다"
    ...
    def process(self) -> Union[str, Tuple[str, str], None]:
        """
        전체 처리 과정을 실행하는 메인 메소드
        
        Returns:
            처리 결과 ('both', (이미지명, 방향), None)
        """
...
def main():
    """메인 실행 함수 - 외부에서 import 시 직접 호출 가능"""
