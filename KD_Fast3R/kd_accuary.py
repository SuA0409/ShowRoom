...
class Evaluator:
    ...
    def chamber_distance(self, use_squared: bool = True, max_points=1000):
        """
        Chamfer Distance 계산 (point 수 제한하여 안정적으로 수행)
        """
    ...
    def chamber_distance(self, use_squared: bool = True):
        """
        1. Chamber Distance (전체 point cloud 간 유사도)

        두 point cloud 간의 구조적 유사성을 측정합니다.
        각 점에서 가장 가까운 점까지의 거리를 양방향으로 계산합니다.

        공식: CD(P,Q) = (1/|P|) * Σ min||p-q||² + (1/|Q|) * Σ min||q-p||²
    ...
    def pointwise_l2_distance(self):
        """
        2. Point-wise L2 Distance (index 정확도)

        Chamber Distance와 동일하지만 명시적으로 분리하여 구현.
        각 대응되는 포인트 간의 직접적인 거리를 측정합니다.
    ...
    def per_axis_mae(self):
        """
        3. Per-axis MAE (각 축별 정확도)

        X, Y, Z 축별로 Mean Absolute Error를 계산합니다.
        어느 축에서 더 큰 오차가 발생하는지 분석할 수 있습니다.

        공식: MAE_x = (1/N) * Σ |xi - x̂i|
    ...
    def self_consistency_check(self, neighbor_threshold: float = 0.1):
        """
        4. Self-consistency (예측의 내부 일관성)

        예측된 point cloud 내에서 이웃 점들 간의 일관성을 체크합니다.
        급격한 변화나 outlier를 감지할 수 있습니다.
    ...
    def compute_ssim_3d(self, grid_size: int = 32):
        ...
        3D point cloud를 3D grid로 voxelize한 후 SSIM을 계산합니다.
        구조적 패턴의 유사성을 측정할 수 있습니다.

        Args:
            gt_pointmap: Teacher 모델 출력 (Ground Truth)
            pred_pointmap: Student 모델 출력 (예측값)
            grid_size: Voxel grid의 크기

        Returns:
            float: 3D SSIM 값 (0~1, 높을수록 유사)
        """
    ...
    def evaluate_all(self, teacher_output: Dict, student_output: Dict) -> Dict:
        ...
        Args:
            teacher_output: Fast3R (Teacher) 모델의 출력
            student_output: ShowRoom (Student) 모델의 출력
